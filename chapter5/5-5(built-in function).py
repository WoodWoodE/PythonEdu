...
print(tuple(map(two_times, [1, 2 , 3 ,4 ])))
...
# max(iterable) : 인수로 반복 가능한 데이터를 입력받아 그 최댓값을 리턴하는 함수이다.
print("{0:=^40}".format("max(iterable)"))

print(max([1,2,3,4,9,5]))
print(max("pythons"))

######################################################################################################################################################

# min(iterable) :  max 함수와 반대로, 인수로 반복 가능한 데이터를 입력받아 그 최솟값을 리턴하는 함수이다.
print("{0:=^40}".format("min(iterable)"))

print(min([1,2,3,4,9,5]))
print(min("pythons"))

######################################################################################################################################################

# oct(x) :  정수를 8진수 문자열로 바꾸어 리턴하는 함수이다.
print("{0:=^40}".format("oct(x)"))
...
# open(filename, [mode]) :  ‘파일 이름’과 ‘읽기 방법’을 입력받아 파일 객체를 리턴하는 함수이다. 읽기 방법(mode)을 생략하면 기본값인 읽기 모드(r)로 파일 객체를 만들어 리턴한다.
print("{0:=^40}".format("open(filename, [mode])"))
...
# f = open("binary_file", "rb") => 이건 조금 사용법을 알아야할듯 싶다 

######################################################################################################################################################

# ord(c) :  문자의 유니코드 숫자 값을 리턴하는 함수이다.
print("{0:=^40}".format("ord(c)"))
...
# pow(x, y) : x를 y제곱한 결괏값을 리턴하는 함수이다.
print("{0:=^40}".format("pow(x, y)"))

print(pow(2,3))

######################################################################################################################################################

# range([start,] stop [,step]) : for 문과 함께 자주 사용하는 함수이다. 이 함수는 입력받은 숫자에 해당하는 범위 값을 반복 가능한 객체로 만들어 리턴한다.
...
print(list(range(0, -10)))      # start인수의 값이 stop인수의 값보다 클때, step을 지정하지 않는다면 빈 값을 전달
print(list(range(0, -10, 2)))   # start인수의 값이 stop인수의 값보다 클때, step을 양수로 지정하면 빈 값을 전달
print(list(range(0, -10, -1)))  # start인수의 값이 stop인수의 값보다 클때는 음수로 step을 지정해주어야 값을 반환 한다.

######################################################################################################################################################

# round(number [,ndigits]) : 숫자를 입력받아 반올림해 리턴하는 함수이다.
...
# sorted(iterable) : 입력 데이터를 정렬한 후 그 결과를 리스트로 리턴하는 함수이다.
...
# sum(iterable) : 입력 데이터의 합을 리턴하는 함수이다.
print(sum([1, 2, 3]))
print(sum((4, 5, 6)))

######################################################################################################################################################

# tuple(iterable) : 반복 가능한 데이터를 튜플로 바꾸어 리턴하는 함수이다. 만약 입력이 튜플인 경우에는 그대로 리턴한다.
print(tuple("abc"))
...
# zip(*iterable) : 동일한 개수로 이루어진 데이터들을 묶어서 리턴하는 함수이다.
